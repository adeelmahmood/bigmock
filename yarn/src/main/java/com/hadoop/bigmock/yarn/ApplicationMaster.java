...
public class ApplicationMaster {
	...
	public ApplicationMaster() throws IOException, ConfigurationException {
		conf = new YarnConfiguration();
		fs = FileSystem.get(conf);

		// load am properties
		amProperties = ConfigurationLoader.loadProperties("am.properties");
		debug = amProperties.containsKey("debug");
	}
	...
	public boolean init(String[] args) throws ParseException {
		...
		if (!env.containsKey(Environment.CONTAINER_ID.name())) {
			...
			// cliParser.getOptionValue("app_attempt_id");
			// appAttemptId =
			// ConverterUtils.toApplicationAttemptId(appAttemptIdStr);
			// } else {
			// throw new
			// IllegalArgumentException("app_attempt_id not set in environment");
			// }
		} else {
		...
		containerJarPath = env.get("CONTAINER_JAR_PATH");
		numContainers = Integer.parseInt(amProperties.getProperty("numContainers", "1"));
		priority = Integer.parseInt(amProperties.getProperty("priority", "0"));
	...
	public boolean run() throws IOException, YarnException {
		...
		containerMem = Integer.parseInt(amProperties.getProperty("containerMem", "128"));
		...
		containerVCores = Integer.parseInt(amProperties.getProperty("containerCores", "1"));
	...
	private class LaunchContainerRunnable implements Runnable {
		...
		public void run() {
			...
			// // set classname
			vargs.add("-jar");
			vargs.add(FilenameUtils.getName(containerJarPath));
